...
class FileField extends Component {
  constructor(props) {
    ...
    _defineProperty(this, "onChange", _ref5 => {
      let _ref5$target = _ref5.target,
          validity = _ref5$target.validity,
          _ref5$target$files = _slicedToArray(_ref5$target.files, 1),
          file = _ref5$target$files[0];

      if (!file) return; // bail if the user cancels from the file browser

      const _this$props = this.props,
            errorMessage = _this$props.errorMessage,
            onChange = _this$props.onChange; // basic validity check
    ...
    _defineProperty(this, "getImagePath", () => {
      const _this$state = this.state,
            dataURI = _this$state.dataURI,
            oldImagePath = _this$state.oldImagePath;

      const _this$getFile = this.getFile(),
            file = _this$getFile.file; // avoid jank during FileReader processing keeping the old image in place
    ...
    _defineProperty(this, "renderUploadButton", () => {
      const uploadButtonLabel = this.props.uploadButtonLabel;
      const _this$state2 = this.state,
            changeStatus = _this$state2.changeStatus,
    ...
    _defineProperty(this, "renderCancelButton", () => {
      ...
      const changeStatus = this.state.changeStatus; // possible states; no case for 'empty' as cancel is not rendered
  ...
  render() {
    ...
    const _this$state3 = this.state,
          changeStatus = _this$state3.changeStatus,
          errorMessage = _this$state3.errorMessage;

    const _this$getFile2 = this.getFile(),
...
const ChangeInfo = _ref7 => {
  let _ref7$status = _ref7.status,
      status = _ref7$status === void 0 ? 'default' : _ref7$status,
